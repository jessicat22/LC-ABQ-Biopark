...
OCCURRENCE_RECODE_MAIN <- function (){
  ...
  countries_table_build <- occurrence_merge()
  foo <<- countries_table_build
  unduplicated_countries <- 
    countries_table_remove_duplicates(countries_table_build)
  countries_table <<- countries_table_add_nationals(unduplicated_countries)
}
...
countries_table_remove_duplicates <- function(x){
  countries_table <- x
...
countries_table_add_nationals <- function(x) {
  # Create temp table
  occ_subnat_temp <- x
