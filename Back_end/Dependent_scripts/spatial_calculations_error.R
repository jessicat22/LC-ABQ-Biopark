...
SPATIAL_CALCULATIONS_MAIN <- function (){
  ...
  GBIF_point_data <- WGSRPD_index(GBIF_point_data)
  GBIF_point_data <- WGSRPD_lvl1_add(GBIF_point_data)
  GBIF_point_data <- WGSRPD_convert(GBIF_point_data)
  hotspot_results <- HOTSPOTS_calculate(GBIF_point_data)
  ...
  REALMS_calculate(GBIF_point_data)
  EOO_max_calculate()
...
# Parameters: x = GBIF_point_data (table from GBIF_download.R), y = 
#                 geospatial data
# Returns: GBIF_point_data (table)
# Throws: none
...
intersection_calculate <- function (x,y){
...
WGSRPD_lvl1_add <- function (x){
  # Create column for level 1
  x$lvl1 <- lvl3$LEVEL1_COD[x$lvl3]
  GBIF_working <- x
  return(GBIF_working)
}
...
WGSRPD_convert <- function(x) {
  # Convert to WGSRPD level 3 code
  x$lvl3 <- lvl3$LEVEL3_COD[x$lvl3]
  # Index WGSRPD level 3 code to return IUCN code
  x$lvl3 <- occ.codes$iucn_code[match(x$lvl3,
...
POINT_data_origin_recalculate <- function(x) {
  ...
  # Convert to long
  GBIF_working <- data.frame(bind_rows(GBIF_working, .id="name"))
  return(GBIF_working)
}
...
HOTSPOTS_calculate <- function (x) {
  # Calculate hotspots for native point occurrences
  GBIF_hotspots <- hotspot.ref(x)
  # Index taxon IDs
  GBIF_hotspots$row.id <- x$ID_NO[GBIF_hotspots$row.id]
  # Remove duplicates
...
REALMS_calculate <- function (x) {
  # Calculate realms for native point occurrences
  GBIF_realms <- realm.ref(x)
  # Index taxon IDs
  GBIF_realms$row.id <- x$ID_NO[GBIF_realms$row.id]
  # Index realms
