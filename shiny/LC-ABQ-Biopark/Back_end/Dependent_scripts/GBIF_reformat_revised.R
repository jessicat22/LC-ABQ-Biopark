...
GBIF_reject <- function(){
  ...
  GBIF_ALL$REJECT_GBIG_flags <- str_detect(gbif_reject_parameters,GBIF_ALL$issue)
  # Find records with implausible year or no year
  GBIF_ALL$REJECT_no_year <- is.na(GBIF_ALL$year) | 
    GBIF_ALL$year < 1700 | 
    GBIF_ALL$year > as.numeric(format(Sys.time(), "%Y"))
  # Identify records with uncertainty over 5km
